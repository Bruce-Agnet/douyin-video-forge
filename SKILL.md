...
首日执行时**必须**下载 1-2 个头部视频并提取关键帧，用于判断竞品内容形态（真人口播 / 情景剧 / 图文混剪 / AI 生成等）。后续每日执行时可跳过（除非策略方向调整）。

```
tikhub_download_video(video_id="<top_video_id>", extract_frames=true)
```
...
6. **内容形态适配性**：基于步骤 8 的视觉分析，判断目标赛道的主流内容形态。评估 AI 视频生成（可灵/Seedance）是否适合该赛道：
